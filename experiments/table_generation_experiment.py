...
class TableGenerationExperiment:
    ...
    async def _execute_with_fallback(self, task: TableTask) -> TableResult:
        ...
        metrics["error_messages"] = error_messages + (metrics.get("error_messages") or [])
    ...
    async def _simple_generation(self, task: TableTask) -> str:
        try:
            ...
            separator = "|" + "|".join(['-' * (len(col) + 2) for col in task.required_columns]) + "|"
        ...
        try:
            if format_type == "markdown":
                ...
                if len(lines) >= 1:
                    ...
                    metrics["structure_score"] = valid_columns / len(columns) if columns else 0.0
        except Exception as e:
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        successful = [r for r in self.results if r.structure_score and r.structure_score > 0.5]
